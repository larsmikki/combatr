...
export function createApp() {
  ...
  app.use(cors())
  // Bulk compendium imports can be large, so allow a generous body size.
  app.use(express.json({ limit: '50mb' }))

  // Request logging — suppress the chatty GET /api/state and /api/health polls
  // when they succeed; still log them when they fail.
  app.use((req: Request, res: Response, next: NextFunction) => {
    const start = Date.now()
    res.on('finish', () => {
      const isQuiet = req.method === 'GET'
        && /^\/api\/(state|health)$/.test(req.path)
        && res.statusCode < 400
      if (!isQuiet) {
        logger.info('request', {
          method: req.method,
